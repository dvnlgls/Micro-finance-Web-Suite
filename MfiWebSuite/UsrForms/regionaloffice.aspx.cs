...
{
    public partial class regionaloffice : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                OfficeID = (string)Page.RouteData.Values[AppRoutes.RegionalOffice.ValOfficeID];
            }
            catch (Exception ex)
            {
            }
            ...
            if (OfficeID != null)
            {
                ...
                string OfficeTypeID = DbSettings.OfficeType.RegionalOffice;
                ...
                if (UserID != null && UserStatusID == DbSettings.UserStatus.Active)
                {
                    ...
                    bool AccessView = MenuActions.Contains(DbSettings.AccessOffice.ViewRoTab);
                    ...
                    if (AccessView)
                    {
                        ...
                        if (CanAccessOffice == CustEnum.PageAccess.Yes)
                        {
                            ...
                            bool AccessEditRO = MenuActions.Contains(DbSettings.AccessOffice.EditRo);
                            bool AccessViewAO = MenuActions.Contains(DbSettings.AccessOffice.ViewAoTab);
                            ...
                            if (AccessView)
                            {
                                ...
                                {
                                    viewRO.InnerHtml = UiMsg.PageRO.ViewRoFetchErr.ErrorWrap();
                                }
                            ...
                            if (AccessViewAO)
                            {
                                DataSet dsSubOff = objOff.GetAllImmediateSubOffices(OfficeID, DbSettings.OfficeType.AreaOffice);
                                ...
                                if (!DataUtils.IsDataSetNull(dsSubOff, 0))
                                {
                                    ...
                                    foreach (DataRow dr in dsSubOff.Tables[0].Rows)
                                    {
                                        ...
                                        string SubOffURL = Page.GetRouteUrl(AppRoutes.AreaOffice.Name, new { OfficeID = dr["OfficeID"].ToString() });
                                ...
                                {
                                    viewAO.InnerHtml = UiMsg.PageRO.ViewAoFetchErr.ErrorWrap();
                                }
                            }
                            else
                            {
