...
namespace MfiWebSuite.UsrForms
{
    public partial class branchoffice : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                OfficeID = (string)Page.RouteData.Values[AppRoutes.BranchOffice.ValOfficeID];
            }
            catch (Exception ex)
            {
            }
            ...
            if (OfficeID != null)
            {
                ...
                string OfficeTypeID = DbSettings.OfficeType.BranchOffice;
                ...
                if (UserID != null && UserStatusID == DbSettings.UserStatus.Active)
                {
                    ...
                    bool AccessView = MenuActions.Contains(DbSettings.AccessOffice.ViewBoTab);
                    ...
                    if (AccessView)
                    {
                        ...
                        if (CanAccessOffice == CustEnum.PageAccess.Yes)
                        {
                            ...
                            bool AccessEditBO = MenuActions.Contains(DbSettings.AccessOffice.EditBo);
                            bool AccessViewVil = MenuActions.Contains(DbSettings.AccessOffice.ViewVilTab);
                            ...
                            if (AccessView)
                            {
                                ...
                                {
                                    viewRO.InnerHtml = UiMsg.PageRO.ViewBoFetchErr.ErrorWrap();
                                }
                            ...
                            if (AccessViewVil)
                            {
                                DataSet dsSubOff = objOff.GetAllImmediateSubOffices(OfficeID, DbSettings.OfficeType.Village);
                                ...
                                if (!DataUtils.IsDataSetNull(dsSubOff, 0))
                                {
                                    ...
                                    foreach (DataRow dr in dsSubOff.Tables[0].Rows)
                                    {
                                        ...
                                        string SubOffURL = Page.GetRouteUrl(AppRoutes.Village.Name, new { OfficeID = dr["OfficeID"].ToString() });
                                ...
                                {
                                    viewSubOff.InnerHtml = UiMsg.PageRO.ViewVilFetchErr.ErrorWrap();
                                }
                            }
                            else
                            {
